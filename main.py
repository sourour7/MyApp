...
def scrape_movies():
    ...
    for container in movie_div:
        ...
        metascore_text = container.find('span', class_='metascore').text.strip() if container.find('span',
                                                                                                   class_='metascore') else '-'
        metascore = int(metascore_text) if metascore_text != '-' else None

        gross_text = container.find('p', class_='sort-num_votes-visible').find_all('span', attrs={'name': 'nv'})[-1][
            'data-value'] if container.find('p', class_='sort-num_votes-visible').find_all('span', attrs={
            'name': 'nv'}) else '-'
        gross = int(gross_text) if gross_text != '-' else None

        votes_text = container.find('p', class_='sort-num_votes-visible').find_all('span', attrs={'name': 'nv'})[0][
            'data-value'] if container.find('p', class_='sort-num_votes-visible').find_all('span', attrs={
            'name': 'nv'}) else '-'
